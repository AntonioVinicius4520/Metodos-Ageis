...
def janela_update():
    campo = ['data', 'codigo', 'quantidade',
             'valor_unitario', 'tipo_op', 'corretagem']
    layout = [
...
def criar_tabela():
    headings2 = ['ID', 'DATA', 'CÓDIGO', 'QUANTIDADE', 'VALOR UNITARIO', 'TIPO OPERAÇÃO',
                 'CORRETAGEM', 'VALOR OPERAÇÃO SEM TAXA', 'TAXA B3', 'VALOR DA OPERAÇÃO']
    valores_tabela2 = []
    layout = [[Text('Pesquisar ação:'), Input(key='Pesquisa', size=(20, 20), do_not_clear=False), Button('Search')],
              [Text('Deletar ação: '), Input(key='Deleta', size=(
                  10, 10), do_not_clear=False), Button('Delete')],
              [Text(key='Tabela'), Table(values=valores_tabela2, headings=headings2,
                                         key='tabela', auto_size_columns=False, def_col_width=15)],
              [Button('Mostrar Tabela'), Button('Cadastro de operações'),
               Button('Fazer Alterções'), Button('Mostrar as médias')]
              ]
...
def cadastro():
    ...
    layout = [
        ...
        [Text('Data: ', text_color='black'), Input(
            size=(10, 10), key='Date', do_not_clear=False)],
        [Text('Código:',  text_color='black',), Input(
            size=(10, 10), key='código', do_not_clear=False)],
        [Text('Quantidade:',  text_color='black'), Input(
            key='quantidade', do_not_clear=False)],
        [Text('Valor unitario: ',  text_color='black'), Input(
            key='valor_unitario', do_not_clear=False)],
        [Text('Tipo de operação',  text_color='black'), DropDown(
            tipo, size=(10, 10), key='Tipo_de_operação', default_value='compra')],
        [Text('Corretagem: ',  text_color='black'), Input(
            key='corretagem', do_not_clear=False)],
        [Button('Salvar'), Button('Fechar Programa'),
         Button('Mostrar Tabela')],
...
# João Emanuel
janela1, janela2, janela3, janela4 = cadastro(), criar_tabela(), None, None
...
while True:
    ...
    if window == janela1 and eventos == 'Salvar':
        ...
        if valores['Date'] == '' or valores['código'] == '' or valores['quantidade'] == '' or valores['Tipo_de_operação'] == '' or valores['valor_unitario'] == '' or valores['corretagem'] == '':
            ...
            try:
                ...
                popup(
                    'Quantidade, valor unitario e corretagem inválido. Digite novamente')
            else:
                datas = valores['Date']
                codigos = valores['código']
                tipo_operacao = valores['Tipo_de_operação']
                banco = BancoOperacao()
                acao = OperacaoB3(datas, codigos, valor,
                                  valor2, tipo_operacao, valor3)
    ...
    if window == janela2 and eventos == 'Delete':
        ...
        if id == '':
            ...
            try:
                ...
                popup(f'A ação que possui o id {id} foi deletado',
                      auto_close=True, auto_close_duration=2)
                janela2['tabela'].update(valores_tabela2)
    if window == janela2 and eventos == 'Fazer Alterções':
        janela3 = janela_update()

    if window == janela2 and eventos == "Mostrar as médias":
        janela4 = tabela_media()
    ...
    if window == janela4 and eventos == WIN_CLOSED:
        janela4.close()

    if window == janela4 and eventos == 'Mostrar Média':
        banco2 = BancoOperacao()
        valor = [banco2.calcular_media_acoes()]
        valores_tabela3 = []
        janela4['media'].update(valores_tabela3)
        for acao in valor:
            for dados in acao:
                valores_tabela3.append(dados)
        janela4['media'].update(valores_tabela3)
